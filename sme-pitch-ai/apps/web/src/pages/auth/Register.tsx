  ...
  if (done) return (
    <div className="min-h-screen bg-light-grey flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md text-center">
        ...
        <p className="text-steel-grey text-sm">We sent a verification link to <strong>{form.email}</strong>.</p>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {(['name', 'email', 'password'] as const).map(field => (
            <div key={field}>
              <label htmlFor={field} className="block text-sm font-medium text-dark-grey mb-1 capitalize">
